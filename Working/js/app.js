...
window.addEventListener('load', () => {
    ...
    }, 100); // Shorter delay for faster UI response
});
...
function checkIfUserHasRsvpd(eventId, eventData) {
    ...
    // Check if user is host - hosts should always be able to RSVP
    const isHost = currentUser && eventData && eventData.createdBy === currentUser.uid;
    if (isHost) {
        // Don't mark as RSVP'd if host, so they can still add items
        userHasRsvpd = false;
        return;
    }
