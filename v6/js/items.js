...
function handleItemFormSubmit() {
    ...
    if (!eventId && window.currentEventId) {
        console.log("Using window.currentEventId as fallback");
...
function showDeleteConfirmation(itemId) {
    ...
    // Try multiple methods to find the correct event ID
    // 1. Try to get it from the event title in the details panel
    ...
    if (detailsPanel) {
        ...
        if (detailTitle) {
            ...
            eventsRef.once('value', (snapshot) => {
                ...
                tryOtherEventIdMethods(modal);
            });
            return;
        }
    }
    
    // If we couldn't find the event title, try other methods
    tryOtherEventIdMethods(modal);
}

// Helper function to try other methods to find event ID
function tryOtherEventIdMethods(modal) {
    // 2. Try to use the global currentEventId from app.js
    ...
    // 3. Look for event ID in existing items
    const itemCard = document.querySelector('.item-card');
    if (itemCard) {
        const itemId = itemCard.getAttribute('data-id');
        if (itemId) {
            // Look up this item to get its event ID
            const itemsRef = database.ref('events');
            itemsRef.once('value', (snapshot) => {
                const events = snapshot.val();
                for (const eventId in events) {
                    const items = events[eventId].items || {};
    ...
    // Last resort - use whatever eventId is in the delete-event-id field
    const existingEventId = document.getElementById('delete-event-id').value;
    if (existingEventId) {
        modal.style.display = 'block';
    } else {
        console.error('Could not determine event ID for deletion');
