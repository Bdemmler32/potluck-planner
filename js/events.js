...
function renderEventDetailView() {
    ...
    eventHeader.querySelector('.back-btn').addEventListener('click', navigateToEventList);
    ...
    // Add Item Button (only for non-past events)
    if (!isPastEvent) {
...
function countItemsByCategory(items, category) {
    return Object.values(items).filter(item => item.category === category).length;
}
